  ...
  switch ( param )
  {
    ...
    case GAPROLE_ADVERT_ENABLED:
      if ( len == sizeof( uint8 ) )
      {
        ...
        {
          gapRole_AdvEnabled = *((uint8*)pValue); // Might be a bug because even issuing a wrong command changes gapRole_advEnabled.
    ...
    case GAPROLE_ADVERT_DATA:
      if ( len <= B_MAX_ADV_LEN )
      {
        ...
        ( gapRole_state != GAPROLE_STARTED ) )
        {
          ...
          GAP_UpdateAdvertisingData( gapRole_TaskID, TRUE,
    ...
    case GAP_END_DISCOVERABLE_DONE_EVENT:
      {
        ...
        if ( pPkt->hdr.status == SUCCESS )
        {
          ...
          {
            if ( gapRole_AdvertOffTime != 0 ) // Restart adv after the "OffTime"
            {
              if ( ( gapRole_AdvEnabled ) )
              {
            ...
            {
              // Waiting after timeout actions will be notified to APP here, after turning of ADV
              if ( gapRole_state == GAPROLE_WAITING_AFTER_TIMEOUT ) 
                ;
    ...
    case GAP_LINK_TERMINATED_EVENT:
      {
        ...
        if ( gapRole_state == GAPROLE_CONNECTED_ADV )
        {
          ...
          GAP_EndDiscoverable( gapRole_TaskID );
          newstate = GAPROLE_CONNECTED;         // Normal exit from CONNECTED_ADV
        }
        ...
        if( pPkt->reason == LL_SUPERVISION_TIMEOUT_TERM )
        {
          ...
          if (gapRole_state == GAPROLE_CONNECTED_ADV)    // Timeout exit from CONNECTED_ADV
            notify = FALSE;    // No notify here
          else
            notify = TRUE;     // Timeout exit from CONNECTED
          //uint8 adv_enable = TRUE;
          //GAPRole_SetParameter( GAPROLE_ADVERT_ENABLED, sizeof( uint8 ), &adv_enable );
        }
        else if (newstate != GAPROLE_CONNECTED) // Normal exit from CONNECTED
        {
          newstate = GAPROLE_WAITING;
          notify = TRUE;
          //uint8 adv_enable = TRUE;
