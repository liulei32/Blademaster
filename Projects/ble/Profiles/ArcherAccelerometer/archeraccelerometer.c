...
// Accelerometer Sample Period UUID
CONST uint8 samplePeriodUUID[ATT_BT_UUID_SIZE] =
{ 
  LO_UINT16(ACCEL_SAMPLEPERIOD_UUID), HI_UINT16(ACCEL_SAMPLEPERIOD_UUID)
};
...
// Accel Sample Period Characteristic Properties
static uint8 accelSamplePeriodCharProps = GATT_PROP_READ | GATT_PROP_WRITE;
// Accel Sample Period Characteristic Value //ms
...
static gattAttribute_t accelAttrTbl[SERVAPP_NUM_ATTR_SUPPORTED] = 
{
  ...
  { 
   ...
   // Z-Coordinate Characteristic Declaration
    ...
    // Accel Sample Period Characteristic Declaration
    { 
      { ATT_BT_UUID_SIZE, characterUUID },
      GATT_PERMIT_READ, 
      0,
      &accelSamplePeriodCharProps 
    },

      // Accelerometer Sample Period Char Value
      { 
        { ATT_BT_UUID_SIZE, samplePeriodUUID },
        GATT_PERMIT_READ | GATT_PERMIT_WRITE, 
        0,
        (uint8 *)&accelSamplePeriod 
      },

      // Accelerometer Sample Period User Description
      { 
        { ATT_BT_UUID_SIZE, charUserDescUUID },
        GATT_PERMIT_READ, 
        0,
...
bStatus_t Accel_GetParameter( uint8 param, void *value )
{
  ...
  switch ( param )
  {
    ...
    case ACCEL_SAMPLEPERIOD:
      *((uint32*)value) = accelSamplePeriod;
      break;
