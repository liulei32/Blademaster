...
typedef struct
{
  HalMotorControl_t HalMotorControlTable[HAL_MOTOR_DEFAULT_MAX_MOTORS];
  uint8           sleepActive;
} HalMotorStatus_t;
...
static uint8 HalMotorState;              // Motor state at last set/clr/blink update

#if HAL_MOTOR == TRUE
static uint8 HalSleepLedState;         // Motor state at last set/clr/blink update
static uint8 preBlinkState;            // Original State before going to blink mode
                                       // bit 0, 1, 2, 3 represent led 0, 1, 2, 3
#endif

#ifdef BLINK_MOTORS
  static HalMotorStatus_t HalMotorStatusControl;
#endif
...
#if (HAL_MOTOR == TRUE)
void HalMotorUpdate (void);
void HalMotorOnOff (uint8 leds, uint8 mode);
#endif /* HAL_MOTOR */
...
{
#if (HAL_MOTOR == TRUE)
  HalMotorSet(HAL_MOTOR_ALL, HAL_MOTOR_MODE_OFF);  // Initialize all Motors to OFF.

  // Set Motor GPIOs to outputs.
  MOTOR_DDR |= MOTOR_BV;

#if defined BLINK_MOTORS
  HalMotorStatusControl.sleepActive = FALSE;  // Initialize sleepActive to FALSE.
#endif
#endif
}
 ...
 * @fn      HalMotorSet
 *
 * @brief   Tun ON/OFF/TOGGLE given Motors
 *
 * @param   motor - bit mask value of leds to be turned ON/OFF/TOGGLE
 *          mode - BLINK, FLASH, TOGGLE, ON, OFF
 * @return  None
 ***************************************************************************************************/
uint8 HalMotorSet (uint8 motors, uint8 mode)
{

#if (defined (BLINK_MOTORS)) && (HAL_MOTOR == TRUE)
  uint8 motor;
  HalMotorControl_t *sts;
